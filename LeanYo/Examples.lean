...
-- P4: whiskering vs `hcomp` with `yoneda` (queue row 24 refined)
-- The literal `(η ◫ yoneda)` is ill-typed: `yoneda` is a `Functor`, not a `NatTrans`.
-- Mathlib bridges via `𝟙 yoneda` and `Functor.hcomp_id` / `Functor.id_hcomp`.

example {C D : Type} [Category C] [Category D] (F G : C ⥤ D) (η : F ⟶ G) (X : C) :
    (η ◫ 𝟙 yoneda).app X = (whiskerRight η yoneda).app X := by
  simp [hcomp_id]

example {C E : Type} [Category C] [Category E]
    (F G : (Cᵒᵖ ⥤ Type v₁) ⥤ E) (η : F ⟶ G) (X : C) :
    (𝟙 yoneda ◫ η).app X =
      (whiskerLeft (F:=yoneda) (G:=F) (H:=G) η).app X := by
  simp [id_hcomp]
