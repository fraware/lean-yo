...
| [MATHLIB_PR_DRAFT_nat_examples.md](MATHLIB_PR_DRAFT_nat_examples.md) | Copy-paste draft for the **first** upstream PR (P1 examples only) |
...
6. **After P1 merges** — return to the queue for P2 (reassoc lemmas, rows 13–18) and P3 (one-shot `@[simp]` bundles, rows 19–23). Row 24 and infra rows stay repo-local (see queue notes).
...
On Linux/macOS: `bash scripts/ci_build.sh` or `make test`.
